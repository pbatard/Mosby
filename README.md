...
### Why do I get a Security Violation when trying to reuse the manufacturer's PK?

If using option `-r` to re-install the default's PK, KEK and DB certs, you may find that
whereas the extra KEK and DB certs provided by the manufacturer will be added successfully,
the PK installation will fail with `Security Violation` and Mosby will fall back to using its
uniquely generated PK. This is because, most platforms that follow the UEFI specs expect the
PK certificate installation package to be signed with the private key that matches the cert,
and, whereas we might be access the cert, if a PKDefault variable exists, we obviously do not
have access to the manufacture's private key.
